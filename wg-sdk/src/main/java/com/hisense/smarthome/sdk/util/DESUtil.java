...
public class DESUtil {
    /*
     * static final鏄〃绀轰簡鍙锛屽垏鑳藉湪鍚屼竴涓繘绋嬬┖闂村唴鐨勫涓?Instance闂村叡浜?
     */
    private static String strDefaultKey = "testtest";
    ...
    /*
     * b2iu鎶奲yte鎸夌収涓嶈?铏戞璐熷彿鐨勫師鍒欑殑锛傚崌浣嶏紓鎴恑nt绋嬪簭锛?鍥犱负java娌℃湁unsigned杩愮畻
     */
     ...
     * 鍥犱负java涓殑byte鐨則oString鏃犳硶瀹炵幇杩欎竴鐐癸紝鎴戜滑鍙堟病鏈塁璇█涓殑
    ...
    /*
     * desMemcpy鏄竴涓唴閮ㄤ娇鐢ㄧ殑byte鏁扮粍鐨勫潡鎷疯礉鍑芥暟锛?浠巌nput鐨刬npos寮?鎶妉en闀垮害鐨?
    ...
    public static void main(String args[]) {
        ...
        DESUtil m = new DESUtil();
        String s = "111111";
        String miwen = m.Encode(s);
        // @System.out.println("瀵嗘枃:[" + miwen + "]");

        String minwen = m.Decode(miwen).toString();
        // @System.out.println("鏄庢枃:[" + minwen + "]");
    }
    ...
    public String Decode(String str) {
        ...
        byte[] byteMingW = new byte[3000]; // 瀵嗘枃
        byte[] byteMiW = new byte[3000];

        DESUtil m = new DESUtil();
        byteMiW = DESUtil.hexStr2ByteArr(str);

        i2 = m.DECRYPT(DESUtil.strDefaultKey.getBytes(), byteMingW, byteMiW,
                byteMiW.length);
        return new String(byteMingW, 0, i2);
    }

    public byte[] tripleDes(String input, String key) {
        byte[] key1 = new byte[8];
        byte[] key2 = new byte[8];
        byte[] output1 = new byte[64];
        byte[] output2 = new byte[64];
        byte[] output3 = new byte[64];
        System.arraycopy(key.getBytes(), 0, key1, 0, 8);
        System.arraycopy(key.getBytes(), 8, key2, 0, 8);
        byte[] src = new byte[64];
        System.arraycopy(input.getBytes(), 0, src, 0, input.getBytes().length);
        this.ENCRYPT(key1, src, output1, 64);
        this.DECRYPT(key2, output2, output1, 64);
        this.ENCRYPT(key1, output2, output3, 64);
        return output3;
    }

    public byte[] tripleDes(byte[] input, byte[] key) {
        byte[] key1 = new byte[8];
        byte[] key2 = new byte[8];
        byte[] output1 = new byte[64];
        byte[] output2 = new byte[64];
        byte[] output3 = new byte[64];
        byte[] output4 = new byte[16];
        System.arraycopy(key, 0, key1, 0, 8);
        System.arraycopy(key, 8, key2, 0, 8);
        byte[] src = new byte[64];
        System.arraycopy(input, 0, src, 0, input.length);
        this.ENCRYPT(key1, src, output1, 64);
        this.DECRYPT(key2, output2, output1, 64);
        this.ENCRYPT(key1, output2, output3, 64);
        System.arraycopy(output3, 0, output4, 0, 16);
        return output4;
    }
}
